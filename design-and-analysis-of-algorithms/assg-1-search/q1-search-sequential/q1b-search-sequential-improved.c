...
long int get_impr_sequential_search_time(int n) {
    // array declared in heap, to avoid space restrictions
    int *arr = malloc(sizeof(int) * (n + 1)), key = -1;
...
int main(int argc, char *argv) {
    ...
    for (int i = 0; i < array_length; i++)
        fprintf(fp, "%ld,%ld\n", array_sizes[i], get_impr_sequential_search_time(array_sizes[i]));
